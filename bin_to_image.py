...
def bins_to_image(input_path, output_path, target_format):
...
def bins_to_images(input_folder, output_folder, target_format):
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            if is_bin(file):
                input_path = os.path.join(root, file)
                output_path = root.replace(input_folder, output_folder)
                bins_to_image(input_path, output_path, target_format)
